...
import { MapPin, Calendar, Plus, X, CloudRain, Thermometer, Wind } from 'lucide-react';
...
function TripSetup() {
    ...
    loader.load().then(() => {
      ...
      destinations.forEach((destination, index) => {
        if (mapDivRefs.current[index] && !destination.mapRef) {
          ...
          // If there's an existing place, search for it
  ...
  const fetchWeather = async (coordinates: { lat: number; lng: number }, index: number) => {
    ...
    try {
      const response = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${coordinates.lat}&lon=${coordinates.lng}&appid=${OPENWEATHER_API_KEY}&units=metric`
      );

      const weatherForecast = response.data.list
        .filter((item: any) => {
          const forecastDate = item.dt_txt.split(' ')[0];
          return forecastDate >= dates.start && forecastDate <= dates.end;
        })
        .map((item: any) => ({
  ...
  // Update weather when dates change
  useEffect(() => {
    destinations.forEach((destination, index) => {
  ...
  const handleDestinationChange = (index: number, value: string) => {
    ...
    // Only proceed with geocoding if the value is not empty and has changed
    if (value.trim() && value !== destinations[index].lastSearched) {
      // Set a timeout to prevent rapid-fire API calls
        ...
        geocoderRef.current?.geocode({ address: value }, (results, status) => {
          if (status === 'OK' && results && results[0]) {
            ...
            setDestinations(prev => {
              ...
              // Only proceed if we have a valid map reference
  ...
  const handleRemoveDestination = (index: number) => {
    if (destinations.length > 1) {
      // Clear the timeout for the removed destination
      if (searchTimeoutRef.current[index]) {
        clearTimeout(searchTimeoutRef.current[index]);
      }
